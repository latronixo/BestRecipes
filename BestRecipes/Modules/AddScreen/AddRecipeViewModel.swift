...
final class AddRecipeViewModel: ObservableObject {
    ...
    func removeIngredient(at index: Int) {
        guard index < (recipe.extendedIngredients?.count ?? 0) else { return }
    ...
    func updateIngredientQuantity(at index: Int, quantity: String) {
        ...
        if let ingredient = recipe.extendedIngredients?[index] {
            recipe.extendedIngredients?[index].original = "\(quantity) \(ingredient.name)"
        }
    }
